...
def _ensure_compatible_protobuf():
    parts = []
    for chunk in _protobuf_version.split("."):
        if not chunk.isdigit():
            break
        parts.append(int(chunk))
    major_minor = tuple(parts[:2]) if len(parts) >= 2 else (0, 0)
    incompatible_version = major_minor >= (3, 21)
    missing_constructor = not hasattr(FieldDescriptor, "_default_constructor")
    if incompatible_version or missing_constructor:
        raise RuntimeError(
            "Detected protobuf version "
            f"{_protobuf_version}. Mediapipe Tasks require protobuf 3.20.x. "
            "Please run `pip install --force-reinstall \"protobuf>=3.20,<3.21\"` "
            "inside your cv-safety environment."
        )
