...
# Ensure `object_protection` package imports work when running this file directly.
...
DANGEROUS_CLASSES = {'tennis racket'}  # 仅关注网球拍作为危险物品
    ...
    def run(self, video_source: int | str = 0) -> None:
        ...
        try:
            while True:
                ...
                if self.workflow_stage == "selection":
                    cup_detections = [
                        det for det in all_detections if det['class_name'] == 'cup'
                    ]
                    self.relic_detections = self.detect_relics(frame, cup_detections)
                    ...
                    relevant_classes = {'cup', 'person', 'tennis racket'}
                    filtered_detections = [
                        det for det in all_detections if det['class_name'] in relevant_classes
                    ]
                    cup_detections = [
                        det for det in filtered_detections if det['class_name'] == 'cup'
                    ]
                    self.relic_detections = self.detect_relics(frame, cup_detections)
                    self.update_tracking(self.relic_detections)
                    self._update_person_detections(filtered_detections)
